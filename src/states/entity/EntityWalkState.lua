...
function checkObjectCollisions(entity)
    if entity.type == 'ghost' or entity.type == 'bat' then
		return false
    end
	for k, object in pairs(entity.room.objects) do
...
function EntityWalkState:update(dt)
    ...
    if self.entity.direction == 'left' then
        ...
        if checkObjectCollisions(self.entity) then
			self.entity.x = self.entity.x + self.entity.walkSpeed * dt
            self.bumped = true
        elseif self.entity.x <= MAP_RENDER_OFFSET_X + TILE_SIZE then
    ...
    elseif self.entity.direction == 'down' then
        ...
        if checkObjectCollisions(self.entity) then
			self.entity.y = self.entity.y - self.entity.walkSpeed * dt
            self.bumped = true
        elseif self.entity.y + self.entity.height >= bottomEdge then
